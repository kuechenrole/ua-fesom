...
if ($firsttime == yes) then
  echo "firsttime=yes: sleep for 300 seconds"
  sleep 300 
  #sleep 2
else
...
if ( $NEWYEAR == $year2wait4 && $NEWDAY == 1) then
   ...
   if (-e "$jobfile") then
     echo "Checking for 'not converged' statement in $jobfile."
     if ( `grep -c "not converged" $jobfile` != 0) then
	 echo "DETECTED 'NOT CONVERGED' STATEMENT! CANCEL CURRENT SLURM JOB."
	 scancel $ID
