...
%function data_out = get_ssh_out(var_ind,idxOld2New)
%    global ncid_in x2dNew y2dNew
%    data_in = netcdf.getVar(ncid_in,var_ind); data_in = data_in(:,end);
%    data_out = data_in(idxOld2New).*0.0;
    %d2m = 111000
    %ii = (x2dNew>4.45*d2m) & (x2dNew<=4.55*d2m) & (y2dNew>0.10*d2m) & (y2dNew<=0.16*d2m) ;
    %data_out(ii) = mean(data_out(ii))
%end
...
function data_out = get_velo_out(var_ind)
    ...
    if lev_diff(i) ~= 0
       ...
       data_out(idx)=data_out(idx)+du*weights;
    end
    end
    %}
    end
