...
if ($2 == 'only') then
 sbatch ua.run.stop
 echo "======================================================"
 echo "launched ua.run.stop; exit after launchua only has been completed"
 echo "======================================================"
 exit
endif
...
if ($NEWYEAR >= $finyear) then
 sbatch ua.run.stop
 echo "==================================================================================="
 echo "launched ua.run.stop; exit after launchua has been completed. Final year ("$finyear") has been reached."
 echo "==================================================================================="
 exit
endif

sed -i "s~masterdir=.*~masterdir='$masterdir'~" ua.run
sbatch ua.run
echo "=========================================================================="
echo "launched ua.run; Ocan2ice part of Mr. Timms script completed; Ua is supposed to run for year"
...
launchice2ocean:
echo "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
echo "Ua terminated successfully and launched MrTimms launchice2ocean."
echo "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"

setenv OLDYEAR `cat $timmsoutdir/$runid.yeartoicemod.dat`
setenv NEWYEAR `cat $timmsoutdir/$runid.yearfromicemod.dat`

ls $uadir/ResultsFiles/0${NEWYEAR}00-Nodes*.mat
if ($status == 0) then
   sleep 2
   echo "Ua results file exists: launch Mr. Timms ice2ocean"
   ./timms.csh ice2ocean all > $timmsoutdir/timms_ice2oce_$NEWYEAR.log &
   echo "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
   echo "MrTimms.csh ice2ocean has been launched."
   echo "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
else
    echo "Ua results does not exist. Mr timms has to exit!"
endif
exit
...
sed -i "s~oldOceFile=.*~oldOceFile='$oldocefile';~" remap2.m
sed -i "s~newOceFile=.*~newOceFile='$newocefile';~" remap2.m
sed -i "s~oldIceFile=.*~oldIceFile='$oldicefile';~" remap2.m
sed -i "s~newIceFile=.*~newIceFile='$newicefile';~" remap2.m
sed -i "s~oldMeshPath=.*~oldMeshPath='$oldmeshdir/';~" remap2.m
sed -i "s~newMeshPath=.*~newMeshPath='$newmeshdir/';~" remap2.m
#matlab.sh -s -M"-nojvm -r run('remap2.m')"
matlab.sh -s -S"-wprod-0304" -M"-nojvm -r run('remap2.m')"
